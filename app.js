...
(function () {
  ...
  function renderProduct() {
    ...
    main.innerHTML =
      ...
      /* На телефоні склад — окремим блоком ПІСЛЯ секції товару, а не в
         ній: секція там займає цілий екран, тож перший екран
         закінчується кнопкою «Замовити» й рядком під нею, а склад
         чекає нижче. Контактів у правій колонці немає свідомо — їх
         відкриває сама кнопка. На десктопі цей блок порожній: склад
         стоїть у лівій колонці. */
      + '<section class="wrap pdp-sklad">' + skladHTML(p, 'mob') + '</section>'
  ...
  /* Кнопка «Назад» у браузері часто не перезавантажує сторінку, а
     піднімає її з bfcache — рівно в тому стані, в якому її покинули.
     А покинули її під плиткою переходу, яка вже затягнула екран
     кольором фону, і код прибирання плитки при такому поверненні не
     запускається: людина бачила порожній білий екран. Тут плитку
     розводимо так само, як при звичайному приході, і скидаємо
     прапорець, щоб наступне завантаження не розіграло прихід,
     якого не було. */
  window.addEventListener('pageshow', function (e) {
    if (!e.persisted) return;
    [].forEach.call(document.querySelectorAll('.tiles'), tilesReveal);
    try { sessionStorage.removeItem(TILE_FLAG); } catch (err) {}
  });
